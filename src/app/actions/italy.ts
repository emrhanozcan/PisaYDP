...
export async function getAllBranchesStats() {
    ...
    const branchesData = await Promise.all(ALL_BRANCHES.map(async (branchCode) => {
...
export async function getAllStudents() {
    ...
    for (const branchCode of ALL_BRANCHES) {
        const students = await db.branchStudents.getByBranch(branchCode);
...
export async function getAllUniversitiesWithStats() {
    ...
    const uniStats = await Promise.all(universities.map(async (uni) => {
        let totalStudents = 0;
