...
import { Star, Search, Download, CheckCircle, Clock, XCircle, Edit2, Trash2, Upload, Plus, Save, X, Mail, Phone, MapPin, GraduationCap, CreditCard, FileText, Users, ChevronLeft, ChevronRight, ChevronDown, Check, Calendar } from 'lucide-react';
...
export default function UniversitiesClient({ universities, allStudents, initialFavorites, userId }: UniversitiesClientProps) {
    ...
    const handleStudentFormSuccess = (savedStudent?: BranchStudent) => {
        if (!savedStudent) {
            // Fallback reload if no data returned (though typically it returns data)
            window.location.reload();
            return;
        }

        const branchName = savedStudent.branchCode ? (BRANCH_NAMES[savedStudent.branchCode as BranchCode] || 'Bilinmiyor') : 'Bilinmiyor';
        const updatedStudent = { ...savedStudent, branchName } as (BranchStudent & { branchName: string });
    ...
    return (
        <div style={{ display: 'flex', gap: '1rem', height: 'calc(100vh - 60px)' }}>
            ...
            <div style={{ flex: 1, display: 'flex', gap: '1rem', overflow: 'hidden' }}>
                {selectedUni ? (
                    <>
                        {/* Student List Sidebar - Collapsible */}
                        <div style={{ width: isStudentListCollapsed ? '60px' : '320px', transition: 'all 0.3s ease', background: isStudentListCollapsed ? 'linear-gradient(to bottom, #eafaf3, #ffffff, #fbf1f1)' : 'white', borderRadius: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', display: 'flex', flexDirection: 'column', overflow: 'visible', flexShrink: 0, position: 'relative' }}>
                            ...
                            <div style={{ padding: '1rem', borderBottom: '1px solid #f0f0f5', opacity: isStudentListCollapsed ? 0 : 1, pointerEvents: isStudentListCollapsed ? 'none' : 'auto', visibility: isStudentListCollapsed ? 'hidden' : 'visible' }}>
                                    ...
                                    <div>
                                        ...
                                        {editingDeadline ? (
                                            <div style={{ display: 'flex', alignItems: 'center', gap: '4px', marginTop: '4px' }}>
                                                ...
                                                <input type="date" value={ddmmyyyyToIso(tempDeadline)} onChange={(e) => setTempDeadline(isoToDdmmyyyy(e.target.value))} style={{ padding: '2px 6px', borderRadius: '4px', border: '1px solid #E67E22', fontSize: '0.7rem', outline: 'none', cursor: 'pointer' }} autoFocus />
                                ...
                                <div style={{ marginTop: '8px' }}>
                                    <select value={branchFilter} onChange={(e) => setBranchFilter(e.target.value)} style={{ width: '100%', padding: '8px', borderRadius: '8px', border: '1px solid #e8e8ef', fontSize: '0.8rem', background: '#fafafc', outline: 'none', cursor: 'pointer', color: '#374151' }}>
                                    ...
                                    <div key={student.id} onClick={() => setSelectedStudent(student)} style={{ padding: '10px 14px', cursor: 'pointer', background: selectedStudent?.id === student.id ? '#f0f4ff' : 'transparent', borderLeft: selectedStudent?.id === student.id ? '3px solid #6C5CE7' : '3px solid transparent', borderBottom: '1px solid #f8f8f8', transition: 'all 0.15s' }}>
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                            ...
                                            <div style={{ flex: 1, minWidth: 0 }}>
                                                ...
                                                <div style={{ fontSize: '0.7rem', color: '#808191', marginTop: '2px' }}>{student.branchName}</div>
                                            </div>
                        ...
                        <div style={{ flex: 1, background: 'white', borderRadius: '16px', boxShadow: '0 2px 12px rgba(0,0,0,0.04)', overflow: 'auto' }}>
                            {selectedStudent ? (
                                <div id="university-student-detail-print-area" style={{ padding: '1.25rem' }}>
                                    {/* Header */}
                                        ...
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                            ...
                                            <div>
                                                ...
                                                <div style={{ display: 'flex', gap: '0.5rem', marginTop: '0.4rem', alignItems: 'center', flexWrap: 'wrap' }}>
                                                    <span style={{ padding: '4px 8px', borderRadius: '8px', fontSize: '0.75rem', fontWeight: 500, background: '#ECFDF5', color: '#065F46' }}>{selectedStudent.branchName} Şubesi</span>
                                    ...
                                    {(selectedStudent.notes || selectedStudent.description) && (
                                        <div style={{ marginTop: '1rem', background: '#fafafc', borderRadius: '12px', padding: '1rem' }}>
                                            <h4 style={{ fontSize: '0.8rem', fontWeight: '600', color: '#666', marginBottom: '0.5rem' }}>Açıklama / Notlar</h4>
                                            {selectedStudent.description && <p style={{ color: '#555', fontSize: '0.85rem', margin: '0 0 0.5rem 0' }}>{selectedStudent.description}</p>}
                                            {selectedStudent.notes && <p style={{ color: '#888', fontSize: '0.8rem', margin: 0, fontStyle: 'italic' }}>Not: {selectedStudent.notes}</p>}
                                        </div>
                                    )}
                                </div>
                            ) : (
