...
import { Search, Users, Check, X, FileText, Info, Shield, MapPin, Calendar, Clock, UserCheck, ChevronLeft, ChevronRight } from 'lucide-react';
...
export default function GuardianServiceClient({ initialStudents, universities }: GuardianServiceClientProps) {
    ...
    return (
        <div style={{ display: 'flex', height: 'calc(100vh - 140px)', gap: '1.5rem' }}>
            ...
            <div style={{ width: isListCollapsed ? '60px' : '380px', transition: 'all 0.3s ease', background: isListCollapsed ? 'linear-gradient(to bottom, #eafaf3, #ffffff, #fbf1f1)' : 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', overflow: 'visible', flexShrink: 0, position: 'relative' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', opacity: isListCollapsed ? 0 : 1, transition: 'opacity 0.2s', pointerEvents: isListCollapsed ? 'none' : 'auto', visibility: isListCollapsed ? 'hidden' : 'visible' }}>
                    {filteredStudents.map(student => {
                        const svcStatus = student.guardianStatus || 'Bekliyor';
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap' }}>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => { e.stopPropagation(); handleUpdateField(student.id, 'guardianStatus', e.target.value); }}
                                            style={{
            ...
            <div style={{ flex: 1, background: 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                {selectedStudent ? (
                    <div id="guardian-student-detail-print-area" style={{ padding: '1.5rem', height: '100%', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                        {/* Header */}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: '1rem' }}>
                            ...
                            <InfoCard title="Vasi Ayrıntıları" color="#00B894" icon={<Shield size={18} />}>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.2rem 2rem' }}>
                                    <InlineEditableRow label="İşlemi Yapan" icon={<UserCheck size={14} color="#808191" />} value={selectedStudent.guardianOperator} field="guardianOperator" onUpdate={(v) => handleUpdateField(selectedStudent.id, 'guardianOperator', v)} />
                                    <InlineEditableRow label="Geliş Tarihi" icon={<Calendar size={14} color="#808191" />} value={selectedStudent.guardianArrivalDate} field="guardianArrivalDate" type="date" onUpdate={(v) => handleUpdateField(selectedStudent.id, 'guardianArrivalDate', v)} />

                                    <InlineEditableRow label="Şehir" icon={<MapPin size={14} color="#808191" />} value={selectedStudent.guardianCity} field="guardianCity" type="select" options={CITIES.map(c => ({ value: c.name, label: c.name }))} onUpdate={(v) => handleUpdateField(selectedStudent.id, 'guardianCity', v)} />

                                    <InlineEditableRow label="Yeri" icon={<MapPin size={14} color="#808191" />} value={selectedStudent.guardianLocation} field="guardianLocation" onUpdate={(v) => handleUpdateField(selectedStudent.id, 'guardianLocation', v)} />
                                    <InlineEditableRow label="Saati" icon={<Clock size={14} color="#808191" />} value={selectedStudent.guardianTime} field="guardianTime" type="time" onUpdate={(v) => handleUpdateField(selectedStudent.id, 'guardianTime', v)} />

                                    <InlineEditableRow label="Durumu" value={selectedStudent.guardianStatus} field="guardianStatus" type="select" options={['Planlandı', 'Tamamlandı', 'İptal', 'Ertelendi'].map(s => ({ value: s, label: s }))} onUpdate={(v) => handleUpdateField(selectedStudent.id, 'guardianStatus', v)} badge color="#FFEAA7" />
                                </div>
                            </InfoCard>
