...
import { Search, Info, Check, X, FileText, MapPin, Home, DollarSign, Calendar, Clock, ChevronLeft, ChevronRight } from 'lucide-react';
...
interface AccommodationClientProps {
    ...
    branchCode?: BranchCode; // Optional, if provided, filter by this branch
}

export default function AccommodationClient({ initialStudents, universities, branchCode }: AccommodationClientProps) {
    ...
    const [branchFilter, setBranchFilter] = useState<string>(branchCode || '');
    ...
    const handleUpdateField = async (id: string, field: keyof BranchStudent, value: any) => {
        ...
        try {
            ...
            // Revert on error
            setStudents(prev => prev.map(s => s.id === id ? targetStudent : s));
            if (selectedStudent?.id === id) setSelectedStudent(targetStudent);
        ...
        if (isEditing) {
            return (
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', minHeight: '28px', ...style }}>
                    <span style={{ color: '#808191', fontSize: '0.8rem', minWidth: '100px' }}>{label}:</span>
    ...
    return (
        <div style={{ display: 'flex', height: 'calc(100vh - 140px)', gap: '1.5rem' }}>
            ...
            <div style={{ width: isListCollapsed ? '60px' : '380px', transition: 'all 0.3s ease', background: isListCollapsed ? 'linear-gradient(to bottom, #eafaf3, #ffffff, #fbf1f1)' : 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', overflow: 'visible', flexShrink: 0, position: 'relative' }}>
                ...
                <div style={{ flex: 1, overflowY: 'auto', opacity: isListCollapsed ? 0 : 1, transition: 'opacity 0.2s', pointerEvents: isListCollapsed ? 'none' : 'auto', visibility: isListCollapsed ? 'hidden' : 'visible' }}>
                    {filteredStudents.map(student => {
                        const svcStatus = student.accommodationStatus || 'Bekliyor';
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{ display: 'flex', alignItems: 'center', gap: '4px', flexWrap: 'wrap' }}>
                                        ...
                                        <select
                                            ...
                                            onChange={(e) => { e.stopPropagation(); handleUpdateField(student.id, 'accommodationStatus', e.target.value); }}
                                            style={{
            ...
            <div style={{ flex: 1, background: 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', overflow: 'hidden', display: 'flex', flexDirection: 'column' }}>
                {selectedStudent ? (
                    <div id="accommodation-student-detail-print-area" style={{ padding: '1.5rem', height: '100%', display: 'flex', flexDirection: 'column', overflowY: 'auto' }}>
                        {/* Header */}
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1.5rem', paddingBottom: '1.5rem', borderBottom: '1px solid #f0f0f5', flexShrink: 0 }}>
                            ...
                            <div className="no-print">
                                <PdfDownloadButton
                                    ...
                                    type="accommodation"
                                    fileName={`${selectedStudent.firstName} ${selectedStudent.lastName} - Konaklama Detay`}
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1fr)', gap: '1rem' }}>
                            ...
                            <InfoCard title="Öğrenci Ayrıntıları" color="#6C5CE7" icon={<FileText size={18} />}>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.2rem 2rem' }}>
                                    ...
                                    <InlineEditableRow label="Program" value={selectedStudent.program} field="program" type="select" options={['Lisans', 'Önlisans', 'Foundation', 'Dil Okulu'].map(p => ({ value: p, label: p }))} onUpdate={(v) => handleUpdateField(selectedStudent.id, 'program', v)} />
                            ...
                            {/* KONAKLAMA AYRINTILARI */}
                            <InfoCard title="Konaklama Ayrıntıları" color="#00B894" icon={<Home size={18} />}>
                                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: '0.2rem 2rem' }}>
                                    <InlineEditableRow label="Şehir" icon={<MapPin size={14} color="#808191" />} value={selectedStudent.accommodationCity} field="accommodationCity" type="select" options={CITIES.map(c => ({ value: c.name, label: c.name }))} onUpdate={(v) => handleUpdateField(selectedStudent.id, 'accommodationCity', v)} />
                                    <InlineEditableRow label="Konaklama Tipi" icon={<Home size={14} color="#808191" />} value={selectedStudent.accommodationType} field="accommodationType" type="select" options={['Ev', 'Oda', 'Yurt', 'Stüdyo Daire'].map(o => ({ value: o, label: o }))} onUpdate={(v) => handleUpdateField(selectedStudent.id, 'accommodationType', v)} />

                                    <InlineEditableRow label="Adres" icon={<MapPin size={14} color="#808191" />} value={selectedStudent.accommodationAddress} field="accommodationAddress" onUpdate={(v) => handleUpdateField(selectedStudent.id, 'accommodationAddress', v)} />
                                    <InlineEditableRow label="Aylık Kira" icon={<DollarSign size={14} color="#808191" />} value={selectedStudent.accommodationMonthlyRent} field="accommodationMonthlyRent" onUpdate={(v) => handleUpdateField(selectedStudent.id, 'accommodationMonthlyRent', v)} />

                                    <InlineEditableRow label="Fark Ödemesi" icon={<DollarSign size={14} color="#808191" />} value={selectedStudent.accommodationDiffPayment} field="accommodationDiffPayment" onUpdate={(v) => handleUpdateField(selectedStudent.id, 'accommodationDiffPayment', v)} />
