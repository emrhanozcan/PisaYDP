...
import { BranchStudent, BranchCode, University, BRANCH_NAMES } from '@/types';
import { Search, Plus, Edit2, Trash2, Mail, Phone, Download, Upload, X, Save, CheckCircle, Clock, XCircle, MapPin, GraduationCap, CreditCard, FileText, Users, ChevronDown, Check, ChevronLeft, ChevronRight } from 'lucide-react';
import PdfDownloadButton from '@/components/common/PdfDownloadButton';
import StudentAvatar from '@/components/common/StudentAvatar';
import { getSession } from '@/app/actions/auth';

interface StudentsClientProps {
    initialStudents: BranchStudent[];
...
const EMPTY_STUDENT: Partial<BranchStudent> = {
    ...
    packageType: '', accommodationService: 'Hayır', supportPackage: 'Hayır', scholarshipPackage: 'Hayır', guardianService: 'Hayır', ydtSupport: 'Hayır',
...
export default function StudentsClient({ initialStudents, universities, branchCode }: StudentsClientProps) {
    const [students, setStudents] = useState<BranchStudent[]>(initialStudents);
    ...
    const getStudentUniversities = (student: BranchStudent) => {
        if (student.educations && student.educations.length > 0) {
            return student.educations.map(e => universities.find(u => u.id === e.universityId)?.name || '-').join(', ');
        }
        return getUniversityName(student.universityId);
    };
    ...
    const handleUpdateEducation = async (index: number, field: string, value: any) => {
        ...
        // Sync legacy fields if it's the first education
        if (index === 0) {
            // We can optionally sync this, but the backend does it too. 
            // Let's just send the full object update to be safe and let backend handle sync.
        }
    ...
    const handleOpenModal = (student?: BranchStudent) => {
        if (student) { setEditingStudent(student); setIsEditing(true); }
        else { setEditingStudent({ ...EMPTY_STUDENT, branchCode }); setIsEditing(false); }
        setIsModalOpen(true);
    };
    ...
    const handleExcelExport = () => {
        ...
        const headers = [
            'Ad Soyad', 'Telefon', 'E-mail', 'Üniversite', 'Bölüm', 'Program', 'Sınıf', 'Sıralama', 'Bloke', 'Kira Kontratı', 'Sonuç', 'Ödeme', 'Pasaport', 'Danışmanlık',
            'Veli Adı', 'Veli Tel', 'Veli Email', 'Ücret', 'Açıklama', 'Notlar'
        ];
        const rows = students.map(s => [
            `${s.firstName} ${s.lastName}`, s.phone || '', s.email || '', getStudentUniversities(s), s.city || '', s.program || '', s.grade || '',
            s.examResult || '', s.visaResult || '', s.selectionResult || '', s.finalStatus || '', s.paymentStatus || 'Bekleniyor', s.passportNo || '', s.supportPackage || '',
            s.parentName || '', s.parentPhone || '', s.parentEmail || '', s.fee || '', s.description || '', s.notes || ''
        ]);
        const csv = [headers.join(','), ...rows.map(r => r.join(','))].join('\n');
        const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
        const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = `${BRANCH_NAMES[branchCode]}_ogrenciler.csv`; a.click();
    };
        ...
        <div style={{ display: 'flex', height: 'calc(100vh - 140px)', gap: '1.5rem' }}>
            ...
            <div style={{ width: isListCollapsed ? '60px' : '380px', transition: 'all 0.3s ease', background: isListCollapsed ? 'linear-gradient(to bottom, #eafaf3, #ffffff, #fbf1f1)' : 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', display: 'flex', flexDirection: 'column', overflow: 'visible', flexShrink: 0, position: 'relative' }}>
                <button
                    ...
                    style={{ position: 'absolute', right: '-16px', top: '50%', transform: 'translateY(-50%)', background: 'white', border: '1px solid #e0e0e0', borderRadius: '50%', width: '32px', height: '32px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#6C5CE7', zIndex: 20, boxShadow: '0 2px 8px rgba(0,0,0,0.15)' }}
                >
                ...
                <div style={{ padding: '1rem', borderBottom: '1px solid #f0f0f5', display: 'flex', flexDirection: 'column', gap: '0.5rem', opacity: isListCollapsed ? 0 : 1, pointerEvents: isListCollapsed ? 'none' : 'auto', transition: 'opacity 0.2s', visibility: isListCollapsed ? 'hidden' : 'visible' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                        ...
                        <button onClick={() => handleOpenModal()} style={{ padding: '8px 14px', background: 'linear-gradient(135deg, #6C5CE7, #a29bfe)', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: '6px', fontSize: '0.8rem', fontWeight: '500' }}><Plus size={14} /> Yeni</button>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '8px' }}>
                        ...
                        <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} style={{ width: '110px', padding: '8px', border: '1px solid #e8e8ef', borderRadius: '8px', fontSize: '0.8rem', background: '#fafafc', cursor: 'pointer', outline: 'none' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                ...
                                <div style={{ flex: 1, minWidth: 0 }}>
                                    ...
                                    <div style={{ fontSize: '0.75rem', color: '#808191', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                                        {getStudentUniversities(student)}
                                    </div>
                                </div>
            ...
            <div style={{ flex: 1, background: 'white', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.05)', overflow: 'auto' }}>
                {selectedStudent ? (
                    <div id="branch-students-detail-print-area" style={{ padding: '1.5rem' }}>
                        {/* Header */}
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                                ...
                                <div>
                                    ...
                                    <div style={{ display: 'flex', gap: '0.75rem', marginTop: '0.5rem', alignItems: 'center' }}>
                                        <span style={{ fontSize: '0.85rem', color: '#808191' }}>{BRANCH_NAMES[selectedStudent.branchCode]} Şubesi</span>
                            ...
                            <InfoCard title="Eğitim Bilgileri" color="#00B894" icon={<GraduationCap size={18} />}>
                                {(selectedStudent.educations && selectedStudent.educations.length > 0) ? (
                                    selectedStudent.educations.map((edu, index) => (
                                        <div key={index} style={{ marginBottom: index < (selectedStudent.educations!.length - 1) ? '1rem' : 0, paddingBottom: index < (selectedStudent.educations!.length - 1) ? '1rem' : 0, borderBottom: index < (selectedStudent.educations!.length - 1) ? '1px dashed #eee' : 'none', display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                            <InlineEditableRow
                                                ...
                                                displayValue={getUniversityName(edu.universityId)}
                                                field="universityId"
                                                type="select"
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.6rem' }}>
                                        <div style={{ padding: '10px', background: '#f5f5f5', borderRadius: '8px', textAlign: 'center', fontSize: '0.85rem', color: '#666' }}>
                                            Henüz eğitim bilgisi eklenmemiş. Düzenle butonunu kullanarak ekleyebilirsiniz.
                                        </div>
                                    </div>
                                )}
                            </InfoCard>
                        ...
                        {/* Description / Notes */}
                        {(selectedStudent.description || selectedStudent.notes) && (
