...
import { Search, Plus, Edit2, Trash2, Mail, Phone, Download, Upload, X, Save, CheckCircle, Clock, XCircle, MapPin, GraduationCap, CreditCard, FileText, Users, ChevronDown, Check } from 'lucide-react';
...
export default function StudentsClient({ initialStudents, universities, branchCode }: StudentsClientProps) {
    ...
    const handleSaveStudent = async () => {
        ...
        const body = isEditing ? editingStudent : { ...editingStudent, branchCode };
        const res = await fetch('/api/branch/students', { method, headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
        if (res.ok) {
            const saved = await res.json();
            if (isEditing) { setStudents(prev => prev.map(s => s.id === saved.id ? saved : s)); setSelectedStudent(saved); }
            else setStudents(prev => [...prev, saved]);
    ...
    const handleExcelExport = () => {
        ...
        const rows = students.map(s => [
            `${s.firstName} ${s.lastName}`, s.phone || '', s.email || '', getUniversityName(s.universityId), s.city || '', s.program || '', s.grade || '',
